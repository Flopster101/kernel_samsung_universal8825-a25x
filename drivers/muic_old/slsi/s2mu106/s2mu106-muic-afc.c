...
#ifdef CONFIG_LEGACY_MUIC_MANAGER
	...
	muic_attached_dev_t new_dev)
{
	...
	if (!legacy_muic_core_hv_is_hv_dev(muic_pdata))
		goto out;
...
#if IS_ENABLED(CONFIG_LEGACY_MUIC_SUPPORT_POWERMETER)
...
static bool _s2mu106_hv_muic_check_afc_enabled(struct s2mu106_muic_data *muic_data)
{
	...
	if (muic_data->afc_check == false) {
		...
		legacy_muic_afc_request_cause_clear();
#if IS_ENABLED(CONFIG_LEGACY_MUIC_MANAGER)
...
#endif
	} else if (legacy_muic_is_enable_afc_request() == false) {
		str = "HV REQUEST DISABLED";
		afc_request_cause = legacy_muic_afc_get_request_cause();
...
static void _s2mu106_hv_muic_dcp_charger_attach(struct s2mu106_muic_data *muic_data)
{
	...
	legacy_muic_core_hv_state_manager(muic_pdata, HV_TRANS_DCP_DETECTED);
}
...
{
	struct muic_platform_data *muic_pdata = muic_data->pdata;
#if IS_ENABLED(CONFIG_LEGACY_MUIC_MANAGER) && !IS_ENABLED(CONFIG_SEC_FACTORY)
...
static void s2mu106_if_hv_muic_fast_charge_communication(void *mdata)
{
	...
	if (muic_pdata->hv_state == HV_STATE_IDLE) {
		...
		legacy_muic_core_hv_state_manager(muic_pdata, HV_TRANS_NO_RESPONSE);
	}
}
...
static void s2mu106_hv_muic_set_ready(struct s2mu106_muic_data* muic_data)
{
	...
	case ATTACHED_DEV_UNSUPPORTED_ID_VB_MUIC:
		if (muic_pdata->hv_state == HV_STATE_IDLE) {
			...
			legacy_muic_core_hv_state_manager(muic_pdata, HV_TRANS_FAST_CHARGE_REOPEN);
		}
		break;
	case ATTACHED_DEV_AFC_CHARGER_PREPARE_MUIC:
...
static int s2mu106_if_reset_hvcontrol_reg(void *mdata)
{
	...
	mutex_lock(&muic_data->afc_mutex);
	legacy_muic_core_hv_init(muic_pdata);
	mutex_unlock(&muic_data->afc_mutex);
...
static int s2mu106_if_vchgin_isr(void *mdata, int voltage)
{
	...
	vchgin = s2mu106_hv_muic_get_vchgin(muic_data);
	if (IS_VCHGIN_9V(vchgin)) {
		legacy_muic_core_hv_state_manager(muic_pdata, HV_TRANS_VBUS_BOOST);
	} else if (IS_VCHGIN_5V(vchgin)) {
		legacy_muic_core_hv_state_manager(muic_pdata, HV_TRANS_VBUS_REDUCE);
	}
...
{
    return legacy_muic_afc_set_voltage(vol);
}
...
static irqreturn_t s2mu106_hv_muic_vdnmon_isr(int irq, void *data)
{
	...
	if (muic_data->is_dp_drive && !vdnmon && vbus) {
		if (muic_data->pdata->afc_disable == false) {
			legacy_muic_core_hv_state_manager(muic_pdata, HV_TRANS_VDNMON_LOW);
		} else {
			s2mu106_hv_muic_set_chgtype_usrcmd(muic_data);
		}
	} else {
...
static irqreturn_t s2mu106_hv_muic_mrxrdy_isr(int irq, void *data)
{
	...
	legacy_muic_core_hv_state_manager(muic_pdata, HV_TRANS_FAST_CHARGE_PING_RESPONSE);
