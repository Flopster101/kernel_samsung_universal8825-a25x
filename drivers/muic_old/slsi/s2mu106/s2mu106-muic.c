...
#if IS_ENABLED(CONFIG_LEGACY_HICCUP_CHARGER)
...
#if defined(CONFIG_LEGACY_MUIC_SUPPORT_PRSWAP)
...
static void s2mu106_muic_dcd_recheck(struct work_struct *work)
{
	...
	if (!muic_data->vbvolt || legacy_muic_core_get_pdic_cable_state(muic_data->pdata)) {
		goto skip_dcd_recheck;
	}
...
static void s2mu106_if_prswap_work(void *mdata, int mode)
{
	...
	switch (mode) {
		case MUIC_PRSWAP_TO_SINK:
			...
			legacy_muic_core_handle_attach(muic_data->pdata, ATTACHED_DEV_USB_MUIC,
				adc, !!vbvolt);
			break;
...
static void s2mu106_if_set_bypass(void *mdata)
{
	...
	if (muic_data->is_sido_vbus_switch) {
		...
		legacy_muic_core_handle_detach(muic_data->pdata);
	}
}
	...
	muic_attached_dev_t new_dev)
{
	...
	if (new_dev == ATTACHED_DEV_NONE_MUIC) {
		/* Detach from PDIC */
		if (legacy_muic_core_get_pdic_cable_state(muic_data->pdata) == false) {
	...
	if (new_dev != ATTACHED_DEV_NONE_MUIC) {
		adc = _s2mu106_muic_get_rid_adc(muic_data);
		vbvolt = _s2mu106_muic_get_vbus_state(muic_data);
		legacy_muic_core_handle_attach(muic_data->pdata, new_dev, adc, !!vbvolt);
...
#if defined(CONFIG_LEGACY_MUIC_HV_SUPPORT_POGO_DOCK)
	int vbus_value = 0;
#endif
...
static int s2mu106_muic_set_overheat_hiccup_mode(struct s2mu106_muic_data *muic_data, bool en)
{
	...
	if (s2mu106_muic_overheat_hiccup_path_change_need(muic_pdata)) {
		if (en == true) {
			...
			legacy_muic_notifier_attach_attached_dev(ATTACHED_DEV_HICCUP_MUIC);
...
#if IS_ENABLED(CONFIG_LEGACY_MUIC_MANAGER)
		legacy_muic_manager_set_legacy_dev(muic_pdata->muic_if, muic_data->new_dev);
#endif
		legacy_muic_core_handle_attach(muic_pdata, muic_data->new_dev,
...
#if IS_ENABLED(CONFIG_LEGACY_MUIC_SUPPORT_PDIC)
static bool s2mu106_muic_is_opmode_typeC(struct s2mu106_muic_data *muic_data)
{
	struct muic_interface_t *muic_if;
...
static irqreturn_t s2mu106_muic_detach_isr(int irq, void *data)
{
	...
	if (s2mu106_muic_is_opmode_typeC(muic_data)) {
		if (!legacy_muic_core_get_pdic_cable_state(muic_data->pdata)) {
			legacy_muic_core_handle_detach(muic_data->pdata);
...
static irqreturn_t s2mu106_muic_rid_chg_isr(int irq, void *data)
{
	...
	if (s2mu106_muic_is_opmode_typeC(muic_data)) {
		...
		if (muic_data->adc == ADC_GND
				&& muic_data->vbvolt
				&& !legacy_muic_core_get_pdic_cable_state(muic_pdata)
...
static int s2mu106_muic_probe(struct platform_device *pdev)
{
	...
	muic_pdata = legacy_muic_core_init(muic_data);
...
#if IS_ENABLED(CONFIG_LEGACY_MUIC_MANAGER)
	muic_if = legacy_muic_manager_init(muic_pdata, muic_data);
#else
	...
	if (muic_data->pdata->init_gpio_cb)
		muic_data->pdata->init_gpio_cb(muic_data->pdata, legacy_get_switch_sel());

	pr_info("%s: usb_path(%d), uart_path(%d)\n", __func__,
		muic_pdata->usb_path, muic_pdata->uart_path);

#if IS_ENABLED(CONFIG_LEGACY_MUIC_SYSFS)
	ret = legacy_muic_sysfs_init(muic_pdata);
	if (ret) {
	...
	if (legacy_get_afc_mode() == CH_MODE_AFC_DISABLE_VAL) {
...
#if defined(CONFIG_LEGACY_MUIC_SUPPORT_POWERMETER)
	ret = legacy_muic_manager_psy_init(muic_if, &pdev->dev);
	if (ret) {
...
fail:
#if IS_ENABLED(CONFIG_LEGACY_MUIC_SYSFS)
	legacy_muic_sysfs_deinit(muic_pdata);
fail_init_sysfs:
#endif
...
static int s2mu106_muic_remove(struct platform_device *pdev)
{
	...
	if (muic_data) {
		...
		if (muic_data->pdata != NULL)
			legacy_muic_sysfs_deinit(muic_data->pdata);
#endif

#if IS_ENABLED(CONFIG_LEGACY_MUIC_MANAGER)
		if (muic_data->if_data != NULL)
			legacy_muic_manager_exit(muic_data->if_data);
#else
		kfree(muic_data->if_data);
#endif
		legacy_muic_core_exit(muic_data->pdata);
