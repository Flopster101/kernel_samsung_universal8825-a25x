...
#if ((IS_MODULE(CONFIG_SEC_PARAM) || IS_ENABLED(CONFIG_SEC_MPARAM)) && !IS_ENABLED(CONFIG_LEGACY_MUIC_USE_MODULE_PARAM))
...
#endif /* BUILTIN CONFIG_LEGACY_MUIC_NOTIFIER */
...
EXPORT_SYMBOL_GPL(legacy_get_switch_sel);

int legacy_get_uart_sel(void)
{
...
out:
	pr_info("%s: legacy_get_uart_sel 0x%x\n", __func__, local_uart_sel);

	return local_uart_sel;
}
EXPORT_SYMBOL_GPL(legacy_get_uart_sel);
...
EXPORT_SYMBOL_GPL(legacy_get_afc_mode);

int legacy_get_pdic_info(void)
{
...
EXPORT_SYMBOL_GPL(legacy_get_pdic_info);
