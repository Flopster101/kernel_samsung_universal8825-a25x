...
#if IS_ENABLED(CONFIG_HV_MUIC_S2MU004_AFC) || IS_ENABLED(CONFIG_LEGACY_MUIC_HV)
int legacy_muic_afc_request_cause_clear(void);
#endif
...
static int muic_handle_cable_data_notification(struct notifier_block *nb,
			unsigned long action, void *data)
{
#if IS_ENABLED(CONFIG_PDIC_NOTIFIER) && IS_ENABLED(CONFIG_LEGACY_MUIC_SUPPORT_PDIC)
	PD_NOTI_ATTACH_TYPEDEF *pnoti = (PD_NOTI_ATTACH_TYPEDEF *)data;
	muic_attached_dev_t attached_dev = pnoti->cable_type;
#else
	muic_attached_dev_t attached_dev = *(muic_attached_dev_t *)data;
...
#if IS_ENABLED(CONFIG_LEGACY_MUIC_NOTIFIER)
	legacy_muic_notifier_register(&dock_notifier_block,
			muic_handle_dock_notification, MUIC_NOTIFY_DEV_DOCK);
	legacy_muic_notifier_register(&cable_data_notifier_block,
...
{
#if IS_ENABLED(CONFIG_LEGACY_MUIC_NOTIFIER)
	legacy_muic_notifier_unregister(&dock_notifier_block);
#endif /* CONFIG_LEGACY_MUIC_NOTIFIER */
...
static int muic_core_detach_charger(struct muic_platform_data *muic_pdata)
{
	...
	if (muic_if != NULL) {
		...
		ret = legacy_muic_afc_request_cause_clear();
	}
#endif
	...
	muic_attached_dev_t new_dev, bool *noti)
{
	...
	case ATTACHED_DEV_JIG_UART_OFF_MUIC:
		if (new_dev != muic_pdata->attached_dev) {
			if (legacy_muic_core_get_pdic_cable_state(muic_pdata)) {
				lc_noti = false;
			}
			pr_info("%s new(%d)!=attached(%d)\n",
				__func__, new_dev, muic_pdata->attached_dev);
...
bool legacy_muic_core_get_pdic_cable_state(struct muic_platform_data *muic_pdata)
{
	struct muic_interface_t *muic_if = (struct muic_interface_t *)muic_pdata->muic_if;
	bool ret = false;
...
EXPORT_SYMBOL(legacy_muic_core_get_pdic_cable_state);

int legacy_muic_core_handle_attach(struct muic_platform_data *muic_pdata,
...
EXPORT_SYMBOL_GPL(legacy_muic_core_handle_attach);

int legacy_muic_core_handle_detach(struct muic_platform_data *muic_pdata)
{
	int ret = 0;
	bool noti = true;
	struct muic_interface_t *muic_if = (struct muic_interface_t *)muic_pdata->muic_if;
...
EXPORT_SYMBOL_GPL(legacy_muic_core_handle_detach);

#if IS_ENABLED(CONFIG_LEGACY_MUIC_HV)
...
bool legacy_muic_core_hv_is_hv_dev(struct muic_platform_data *muic_pdata)
{
	bool ret = false;
...
EXPORT_SYMBOL_GPL(legacy_muic_core_hv_is_hv_dev);
...
int legacy_muic_core_hv_state_manager(struct muic_platform_data *muic_pdata,
...
EXPORT_SYMBOL_GPL(legacy_muic_core_hv_state_manager);

void legacy_muic_core_hv_init(struct muic_platform_data *muic_pdata)
{
...
EXPORT_SYMBOL_GPL(legacy_muic_core_hv_init);

int legacy_muic_hv_charger_init(void)
{
...
EXPORT_SYMBOL(legacy_muic_hv_charger_init);

#ifdef CONFIG_LEGACY_HV_MUIC_VOLTAGE_CTRL
void legacy_hv_muic_change_afc_voltage(int tx_data)
{
	struct muic_interface_t *muic_if;
...
EXPORT_SYMBOL_GPL(legacy_hv_muic_change_afc_voltage);

int legacy_muic_afc_get_voltage(void)
{
	struct muic_interface_t *muic_if;
	int ret;
...
EXPORT_SYMBOL_GPL(legacy_muic_afc_get_voltage);

int legacy_muic_afc_set_voltage(int vol)
{
	struct muic_interface_t *muic_if;
	int ret;
...
EXPORT_SYMBOL_GPL(legacy_muic_afc_set_voltage);
...
EXPORT_SYMBOL_GPL(legacy_muic_afc_request_cause_clear);

int legacy_muic_afc_get_request_cause(void)
{
	struct muic_interface_t *muic_if;
	int ret = -ENOENT;
...
EXPORT_SYMBOL_GPL(legacy_muic_afc_get_request_cause);

bool legacy_muic_is_enable_afc_request(void)
{
	struct muic_interface_t *muic_if;
	bool ret = 0;
...
EXPORT_SYMBOL_GPL(legacy_muic_is_enable_afc_request);
...
int legacy_muic_afc_request_voltage(int cause, int voltage)
{
	struct muic_interface_t *muic_if;
	int set_vol = 0, ret = 0;
	...
	ret = legacy_muic_afc_set_voltage(set_vol);
out:
	return ret;
}
EXPORT_SYMBOL_GPL(legacy_muic_afc_request_voltage);
#endif /* CONFIG_LEGACY_HV_MUIC_VOLTAGE_CTRL */
#endif

int legacy_muic_set_hiccup_mode(int on_off)
{
	struct muic_interface_t *muic_if;
	int ret = -ENODEV;
...
EXPORT_SYMBOL_GPL(legacy_muic_set_hiccup_mode);

struct muic_platform_data *legacy_muic_core_init(void *drv_data)
{
...
EXPORT_SYMBOL_GPL(legacy_muic_core_init);

void legacy_muic_core_exit(struct muic_platform_data *muic_pdata)
{
	kfree(muic_pdata);
}
EXPORT_SYMBOL(legacy_muic_core_exit);
