	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncasecmp(buf, "1", 1))
		legacy_muic_core_handle_attach(muic_pdata, new_dev, 0, 0);
	else if (!strncasecmp(buf, "0", 1))
		legacy_muic_core_handle_detach(muic_pdata);
	else
		pr_info("%s invalid value\n", __func__);
...
#if IS_ENABLED(CONFIG_LEGACY_MUIC_SYSFS_SHOW_REFRESH_ADC)
...
#if IS_ENABLED(CONFIG_LEGACY_MUIC_HV) && !IS_ENABLED(CONFIG_HV_MUIC_AFC_DISABLE_ENFORCE)
...
#if IS_BUILTIN(CONFIG_LEGACY_MUIC_NOTIFIER)
...
#if IS_ENABLED(CONFIG_LEGACY_HV_MUIC_VOLTAGE_CTRL)
...
#endif /* CONFIG_LEGACY_MUIC_HV  && !CONFIG_HV_MUIC_AFC_DISABLE_ENFORCE */
...
#endif /* CONFIG_LEGACY_HICCUP_CHARGER */
...
EXPORT_SYMBOL_GPL(legacy_muic_sysfs_init);

void legacy_muic_sysfs_deinit(struct muic_platform_data *muic_pdata)
{
	if (muic_pdata->switch_device)
		sysfs_remove_group(&muic_pdata->switch_device->kobj, &muic_sysfs_group);
}
EXPORT_SYMBOL_GPL(legacy_muic_sysfs_deinit);
