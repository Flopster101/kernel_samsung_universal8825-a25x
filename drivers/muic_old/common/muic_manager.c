...
void legacy_muic_manager_set_legacy_dev(struct muic_interface_t *muic_if, int new_dev)
{
	pr_info("%s: %d->%d\n", __func__, muic_if->legacy_dev, new_dev);

	muic_if->legacy_dev = new_dev;
}
EXPORT_SYMBOL(legacy_muic_manager_set_legacy_dev);
...
static int muic_manager_handle_pdic_attach(struct muic_interface_t *muic_if, void *data)
{
	...
	if (pdic->pdic_evt_attached == MUIC_PDIC_NOTI_ATTACH) {
		...
		} else if (vbus && !legacy_muic_core_get_pdic_cable_state(pdata)) {
...
struct muic_interface_t *legacy_muic_manager_init(void *pdata, void *drv_data)
{
#ifdef CONFIG_IFCONN_NOTIFIER
	int ret;
#endif
	...
	muic_if->opmode = legacy_get_pdic_info() & 0xF;
	muic_if->is_pdic_attached = false;
...
EXPORT_SYMBOL(legacy_muic_manager_init);
...
int legacy_muic_manager_psy_init(struct muic_interface_t *muic_if, struct device *parent)
{
...
EXPORT_SYMBOL(legacy_muic_manager_psy_init);
#endif

void legacy_muic_manager_exit(struct muic_interface_t *muic_if)
{
#if IS_ENABLED(CONFIG_LEGACY_MUIC_SUPPORT_POWERMETER)
...
EXPORT_SYMBOL(legacy_muic_manager_exit);
