...
void legacy_muic_notifier_set_new_noti(bool flag)
{
	muic_uses_new_noti = flag ? 1 : 0;
}
EXPORT_SYMBOL(legacy_muic_notifier_set_new_noti);

static void __set_noti_cxt(int attach, int type)
{
	if (type < 0) {
		muic_notifier.cmd = attach;
#if IS_ENABLED(CONFIG_LEGACY_MUIC_MANAGER) && IS_ENABLED(CONFIG_PDIC_NOTIFIER)
...
int legacy_muic_notifier_register(struct notifier_block *nb, notifier_fn_t notifier,
...
EXPORT_SYMBOL(legacy_muic_notifier_register);

int legacy_muic_notifier_unregister(struct notifier_block *nb)
{
	int ret = 0;
...
EXPORT_SYMBOL(legacy_muic_notifier_unregister);
...
static int muic_notifier_notify(void)
{
...
#if defined(CONFIG_SEC_FACTORY)
#if IS_ENABLED(CONFIG_ANDROID_SWITCH) || IS_ENABLED(CONFIG_SWITCH)
#if IS_ENABLED(CONFIG_LEGACY_MUIC_SUPPORT_PDIC) && IS_ENABLED(CONFIG_PDIC_NOTIFIER)
	...
	if (muic_notifier.cmd != 0)
		...
		send_muic_cable_intent(0);
#endif	/* CONFIG_LEGACY_MUIC_SUPPORT_PDIC */
#endif  /* CONFIG_ANDROID_SWITCH || CONFIG_SWITCH */
...
int legacy_muic_pdic_notifier_register(struct notifier_block *nb, notifier_fn_t notifier,
			muic_notifier_device_t listener)
{
	int ret = 0;
#if IS_ENABLED(CONFIG_LEGACY_MUIC_MANAGER) && IS_ENABLED(CONFIG_PDIC_NOTIFIER)
...
EXPORT_SYMBOL(legacy_muic_pdic_notifier_register);

int legacy_muic_pdic_notifier_unregister(struct notifier_block *nb)
{
	int ret = 0;
...
EXPORT_SYMBOL(legacy_muic_pdic_notifier_unregister);
...
void legacy_muic_notifier_attach_attached_dev(muic_attached_dev_t new_dev)
{
	pr_info("%s: (%d)\n", __func__, new_dev);
...
EXPORT_SYMBOL(legacy_muic_notifier_attach_attached_dev);

void legacy_muic_pdic_notifier_attach_attached_dev(muic_attached_dev_t new_dev)
{
	pr_info("%s: (%d)\n", __func__, new_dev);
...
EXPORT_SYMBOL(legacy_muic_pdic_notifier_attach_attached_dev);

void legacy_muic_pdic_notifier_detach_attached_dev(muic_attached_dev_t new_dev)
{
	pr_info("%s: (%d)\n", __func__, new_dev);
...
EXPORT_SYMBOL(legacy_muic_pdic_notifier_detach_attached_dev);

void legacy_muic_notifier_detach_attached_dev(muic_attached_dev_t cur_dev)
{
	pr_info("%s: (%d)\n", __func__, cur_dev);
...
EXPORT_SYMBOL(legacy_muic_notifier_detach_attached_dev);

void legacy_muic_notifier_logically_attach_attached_dev(muic_attached_dev_t new_dev)
{
	pr_info("%s: (%d)\n", __func__, new_dev);
...
EXPORT_SYMBOL(legacy_muic_notifier_logically_attach_attached_dev);

void legacy_muic_notifier_logically_detach_attached_dev(muic_attached_dev_t cur_dev)
{
	pr_info("%s: (%d)\n", __func__, cur_dev);
...
EXPORT_SYMBOL(legacy_muic_notifier_logically_detach_attached_dev);
