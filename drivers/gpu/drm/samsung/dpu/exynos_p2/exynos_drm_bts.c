...
u64 p2_dpu_bts_calc_aclk_disp(struct decon_device *decon,
...
static void dpu_bts_find_max_disp_freq(struct decon_device *decon)
{
	...
	for (i = 0; i < decon->win_cnt; ++i) {
		...
		freq = p2_dpu_bts_calc_aclk_disp(decon, &config[i],
	...
	if (config->state != DPU_WIN_STATE_DISABLED) {
		freq = p2_dpu_bts_calc_aclk_disp(decon, config,
				(u64)decon->bts.resol_clk, disp_op_freq);
		if (disp_op_freq < freq)
			disp_op_freq = freq;
	}
...
struct dpu_bts_ops p2_usdm_dpu_bts_control = {
