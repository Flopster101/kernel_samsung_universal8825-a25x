...
#ifdef CONFIG_LEGACY_MUIC_HV_FORCE_LIMIT
enum {
	HV_9V = 0,
	HV_5V,
};
...
#if defined(CONFIG_LEGACY_MUIC_HV)
/* MUIC HV State type */
typedef enum {
	HV_STATE_INVALID = -1,
	HV_STATE_IDLE = 0,
...
#else
#define MUIC_SEND_NOTI_ATTACH(dev)	\
		legacy_muic_notifier_attach_attached_dev(dev)
#define MUIC_SEND_NOTI_DETACH(dev) \
		legacy_muic_notifier_detach_attached_dev(dev)
#define MUIC_SEND_NOTI_TO_PDIC_ATTACH(dev) \
		legacy_muic_pdic_notifier_attach_attached_dev(dev)
#define MUIC_SEND_NOTI_TO_PDIC_DETACH(dev) \
		legacy_muic_pdic_notifier_detach_attached_dev(dev)
#endif
...
#else
static inline int legacy_muic_hv_charger_init(void) {return 0; }
#endif

int legacy_get_switch_sel(void);
int legacy_get_afc_mode(void);
...
extern bool legacy_muic_core_get_pdic_cable_state(struct muic_platform_data *muic_pdata);
extern struct muic_platform_data *legacy_muic_core_init(void *drv_data);
extern void legacy_muic_core_exit(struct muic_platform_data *muic_pdata);
...
extern int legacy_muic_afc_set_voltage(int voltage);
extern int legacy_muic_afc_request_voltage(int cause, int voltage);
extern int legacy_muic_afc_request_cause_clear(void);
